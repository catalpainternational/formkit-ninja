...
- `schema_emit` — a form's schema as values keyed by the path of node names, never by id:
  `SchemaNode`, `SchemaSnapshot`, `SchemaChange`, `SchemaEvent`, `ChangeKind`, `EventKind`,
  the `SchemaNodeRecord` / `SchemaSnapshotRecord` / `SchemaChangeRecord` JSON shapes,
  `SchemaStreamSink`, `schema_nodes`, `snapshot_schema`, `diff_schema`, `emit_schema`,
  `apply_schema_events`, `append_schema_events`, `schema_event_from_record`,
  `schema_stream_path` and `SCHEMA_PREFIX`. Provisional until a consumer has used it; a schema
