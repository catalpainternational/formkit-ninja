...
def set_initial_status(apps, schema_editor):
    ...
    PublishedForm.objects.filter(is_active=False, replaced__isnull=False).update(status="replaced")

    # Update all active forms as published
    PublishedForm.objects.filter(is_active=True).update(status="published")
    ...
    operations = [
        migrations.AddField(
            ...
            field=models.CharField(
                choices=[("draft", "Draft"), ("published", "Published"), ("replaced", "Replaced")],
                default="draft",
