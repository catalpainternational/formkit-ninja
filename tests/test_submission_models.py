...
class TestSubmissionLifecycle:
    def test_create_submission_flattens_data(self):
        ...
        repeaters = SeparatedSubmission.objects.filter(submission=sub, repeater_key="repeater").with_repeater_order().order_by("repeater_order")
...
@pytest.mark.django_db
class TestStatusSyncTrigger:
    """``SeparatedSubmission.status`` mirrors the root ``Submission.status``; the DB
    trigger (migration 0046) keeps it in sync even for writes that bypass ``save()``."""
