...
def test_a_late_registered_nodes_field_is_not_misfiled_as_an_extra_prop():
    ...
    The assertion has to reach `additional_props`. Asserting only that the key
    set contains ``stepSize`` restates the test above and passes whether or not
    a parsed node files the field correctly — which is the whole defect.
    """
    from formkit_ninja import formkit_schema as fs
    from formkit_ninja import schema_props

    schema_props.recognised_node_prop_keys()  # warm the cache before registering

    class SliderNode(fs.FormKitSchemaProps):
        formkit: str = "slider"
        stepSize: int | None = None

    # `strip_stale_recognised_props` is what the key set actually feeds: it drops a
    # stored extra prop when the same key is a recognised field and the node itself
    # carries a value for it. With a stale cache `stepSize` is not recognised, so the
    # stale duplicate survives and later wins over the edited value — issue #71.
    assert schema_props.strip_stale_recognised_props({"stepSize": 1}, {"stepSize": 5}) == {}
