...
def test_element():
    ...
    """
    node_in_db = FormKitSchemaNode.objects.create(node_type="$el", node={"$el": "div"}, label="A node")
    node_from_db = node_in_db.to_pydantic()
    assert isinstance(node_from_db.root, FormKitSchemaDOMNode)


def test_validate():
    assert isinstance(DiscriminatedNodeType.model_validate({"$el": "div"}).root, FormKitSchemaDOMNode)
