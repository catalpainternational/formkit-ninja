...
class RegisteredTextNode(TextNode):
    """A stand-in for a consumer's own node class, registered against ``text``.

    It has to be a class ``default_registry`` does *not* map ``text`` to. An
    earlier version of the test below registered ``TextNode`` itself, which is
    what the default registry already answers with — so the assertion held
    whether or not the given registry was consulted, and the test could not
    fail. Reverting the fix left the whole suite green.
    """
...
def test_factory_uses_the_registry_it_was_given() -> None:
    ...
    registry.register_formkit_node("text", RegisteredTextNode)

    node = FormKitNodeFactory(registry).from_dict({"$formkit": "text", "name": "f"})

    # `default_registry` answers `text` with `TextNode`, so this is only a
    # `RegisteredTextNode` if the registry passed to `__init__` was the one read.
    assert type(node) is RegisteredTextNode
